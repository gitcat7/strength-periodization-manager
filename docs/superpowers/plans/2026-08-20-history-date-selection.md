...
- [x] **Step 1:** 运行历史页 focused tests 并记录 RED→GREEN。
- [x] **Step 2:** 运行 `pnpm test`。
- [x] **Step 3:** 运行 `pnpm release:check`，确认 typecheck、生产构建和本地 14 路由 smoke 通过。
- [x] **Step 4:** `git diff --check`、确认只改历史页测试/实现与本计划文档。
