      ...
      <section className="mx-auto max-w-3xl space-y-5">
        ...
        <section className={`action-surface p-4 ${nextWorkoutMeta?.intent === "强度" ? "tone-intensity" : ""}`}>
          <div className="mb-4 flex items-center justify-between gap-3">
            ...
            <Link className="pressable inline-flex h-11 items-center gap-1 rounded-md bg-action px-4 text-sm font-semibold text-white" href={nextWorkout ? "/today" : "/plan"} title={nextWorkout ? "继续今日计划" : "创建周期计划"}>
              {nextWorkout ? "继续训练" : "创建周期计划"}
              <ArrowRight size={16} />
            </Link>
          </div>
          {nextWorkout ? (
