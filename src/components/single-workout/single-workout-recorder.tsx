...
type CompletedWorkout = { id: string; summary: Summary; durationSeconds: number | null };
...
export function SingleWorkoutRecorder() {
  ...
  useEffect(() => {
    void createBrowserSupabaseClient().rpc("get_standalone_workout_draft").then(({ data, error }) => {
      ...
      const stored = data as { workout_id?: unknown; exercises?: unknown; started_at?: unknown };
      ...
      setStartedAt(typeof stored.started_at === "string" ? stored.started_at : null);
      if (stored.workout_id) setMessage("已恢复上次未完成的自由训练草稿。");
    });
  }, []);

  useEffect(() => {
    if (!startedAt || completedWorkout) { setElapsedSeconds(null); return; }
    const update = () => setElapsedSeconds(getElapsedDurationSeconds(startedAt));
    update(); const timerId = window.setInterval(update, 1000);
    return () => window.clearInterval(timerId);
  ...
  function payloadItemsFor(items: SelectedExercise[]) { return items.map((item) => item.kind === "reviewed" ? { reviewedExercise: item.reviewed, sets: item.sets } : item.kind === "wger" ? { externalReference: item.reference, sets: item.sets } : { manualExercise: item.manual, sets: item.sets }); }
  async function ensureStandaloneStarted(nextSelected: SelectedExercise[]) {
    if (startedAt || startRequestRef.current) return;
    startRequestRef.current = true;
    const localStartedAt = new Date().toISOString();
    const { data, error } = await createBrowserSupabaseClient().rpc("save_standalone_workout", { p_payload: buildStandaloneWorkoutSavePayload(today, payloadItemsFor(nextSelected), draftWorkoutId ?? undefined, localStartedAt) });
    startRequestRef.current = false;
    if (error || !data || typeof data !== "object") { setMessage("训练开始时间保存失败，请检查网络后重试。"); return; }
    const result = data as { workout_id?: unknown; started_at?: unknown };
  ...
  async function save(status: "draft" | "completed") {
    ...
    const mustUseManualDuration = manualDurationMode || !startedAt;
    const manualDuration = status === "completed" && mustUseManualDuration ? validateManualDurationMinutes(manualDurationMinutes) : null;
    if (status === "completed" && mustUseManualDuration && !manualDuration?.ok) { setSaving(null); setMessage(manualDuration?.message ?? "训练时长请输入 1–720 的整数分钟。"); return; }
    if (status === "completed" && !startedAt && !manualDuration) { setSaving(null); setMessage("尚未记录开始时间，请填写实际训练时长。"); return; }
    const payloadItems = payloadItemsFor(selected);
    const { data, error } = await createBrowserSupabaseClient().rpc("save_standalone_workout", { p_payload: { ...buildStandaloneWorkoutSavePayload(today, payloadItems, draftWorkoutId ?? undefined, startedAt ?? undefined, manualDuration?.ok ? manualDuration.seconds : undefined), status } });
    setSaving(null);
    ...
    if (status === "completed") {
      ...
      setRestSetKey(null);
      setRestRemaining(0);
      setCompletedWorkout({ id: workoutId, summary, durationSeconds: typeof result?.duration_seconds === "number" ? result.duration_seconds : null });
      return;
    }
  ...
  if (completedWorkout) {
    ...
    return <main className="mx-auto max-w-3xl px-4 py-5 pb-[calc(6rem+env(safe-area-inset-bottom))]">
      ...
      <section className="mt-5 border border-line bg-white p-4">
        ...
        <p className="mt-2 text-sm text-muted">训练时长：{completedWorkout.durationSeconds === null ? "未记录" : `${Math.max(1, Math.round(completedWorkout.durationSeconds / 60))} 分钟`}</p>
  ...
  return <main className="mx-auto max-w-3xl px-4 py-5 pb-[calc(6rem+env(safe-area-inset-bottom))]">
    <Link href="/" className="inline-flex items-center gap-1 text-sm text-muted"><ArrowLeft size={16} />返回首页</Link>
    <header className="mt-4 flex items-start justify-between gap-3"><div><p className="page-kicker">自由训练 · {today}</p><h1 className="text-2xl font-bold">记录今日训练</h1><p className="mt-1 text-sm text-muted">本次记录会保存到历史与进展，不会自动调整你的周期计划。</p>{elapsedSeconds !== null ? <p className="mt-2 text-sm font-semibold text-action">已训练 {formatElapsedTime(elapsedSeconds)}</p> : null}</div><button disabled={saving !== null} onClick={requestComplete} className="pressable shrink-0 rounded-md bg-action px-3 py-2 text-sm font-semibold text-white disabled:opacity-60">完成训练</button></header>
