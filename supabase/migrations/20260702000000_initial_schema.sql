-- Baseline for clean local Supabase projects. Existing hosted projects can run
-- this safely because every table/index/policy is idempotent and core actions
...
create table if not exists public.programs (
  ...
  user_id uuid not null references auth.users(id) on delete cascade,
  name text not null,
  template_type text not null check (template_type in ('three_day_full_body', 'four_day_upper_lower')),
...
values
  ('back_squat', '深蹲', 'squat', 5.00, true),
  ('bench_press', '卧推', 'push', 2.50, true),
  ('deadlift', '硬拉', 'pull', 5.00, true),
  ('overhead_press', '推举', 'push', 2.50, true),
  ('barbell_row', '杠铃划船', 'pull', 2.50, false),
  ('pull_up', '引体向上', 'pull', 2.50, false),
  ('lat_pulldown', '高位下拉', 'pull', 2.50, false),
  ('romanian_deadlift', '罗马尼亚硬拉', 'squat', 5.00, false),
  ('leg_press', '腿举', 'squat', 5.00, false),
  ('leg_curl', '腿弯举', 'squat', 2.50, false)
...
create policy "Users manage own athlete profile" on public.athlete_profiles for all to authenticated using (auth.uid() = user_id) with check (auth.uid() = user_id);
drop policy if exists "Users manage own lift profiles" on public.lift_profiles;
create policy "Users manage own lift profiles" on public.lift_profiles for all to authenticated using (auth.uid() = user_id) with check (auth.uid() = user_id);
drop policy if exists "Users manage own programs" on public.programs;
create policy "Users manage own programs" on public.programs for all to authenticated using (auth.uid() = user_id) with check (auth.uid() = user_id);
...
create policy "Users manage own workout exercises" on public.workout_exercises for all to authenticated using (exists (select 1 from public.workouts where workouts.id = workout_exercises.workout_id and workouts.user_id = auth.uid())) with check (exists (select 1 from public.workouts where workouts.id = workout_exercises.workout_id and workouts.user_id = auth.uid()));
drop policy if exists "Users manage own set logs" on public.set_logs;
create policy "Users manage own set logs" on public.set_logs for all to authenticated using (exists (select 1 from public.workout_exercises join public.workouts on workouts.id = workout_exercises.workout_id where workout_exercises.id = set_logs.workout_exercise_id and workouts.user_id = auth.uid())) with check (exists (select 1 from public.workout_exercises join public.workouts on workouts.id = workout_exercises.workout_id where workout_exercises.id = set_logs.workout_exercise_id and workouts.user_id = auth.uid()));
...
create policy "Users manage own PR goals" on public.pr_goals for all to authenticated using (auth.uid() = user_id) with check (auth.uid() = user_id);
