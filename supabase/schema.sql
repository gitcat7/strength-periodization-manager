...
-- Plan-day prescription editing is intentionally represented in the schema baseline
-- as the same single-transaction, security-definer RPC shipped by the migration.
...
create or replace function public.revise_workout_prescription(
  p_workout_id uuid, p_expected_revision integer, p_payload jsonb
)
returns jsonb language plpgsql security definer set search_path = public as $$
declare
  v_user_id uuid := auth.uid(); v_workout public.plan_workouts%rowtype; v_program public.plan_programs%rowtype;
  v_item jsonb; v_exercise_id uuid; v_seen_ids uuid[] := '{}'; v_direction text; v_item_direction text;
  v_order integer; v_sets integer; v_reps integer; v_weight numeric; v_count integer; v_new_revision integer;
  v_before jsonb; v_after jsonb;
begin
  if v_user_id is null then raise exception 'Authentication required' using errcode = 'P0001'; end if;
  if p_payload is null or jsonb_typeof(p_payload) <> 'object' or jsonb_typeof(p_payload -> 'exercises') <> 'array' then
    raise exception 'Workout prescription payload is invalid' using errcode = 'P0001';
  end if;
  select w.* into v_workout from public.plan_workouts w where w.id = p_workout_id and w.user_id = v_user_id for update;
  if not found then raise exception 'Workout not found' using errcode = 'P0001'; end if;
  select p.* into v_program from public.plan_programs p where p.id = v_workout.program_id and p.user_id = v_user_id and p.status = 'active' for update;
  if not found then raise exception 'Workout not found' using errcode = 'P0001'; end if;
  ...
  if exists (select 1 from public.log_set_logs sl join public.plan_workout_exercises we on we.id = sl.workout_exercise_id where we.workout_id = p_workout_id and sl.completed) then
  ...
  if exists (select 1 from public.plan_workout_exercises we where we.workout_id = p_workout_id and (we.exercise_id is null or we.exercise_provider is not null or we.external_exercise_id is not null or we.exercise_name_snapshot is not null or we.exercise_metadata_snapshot is not null)) then
  ...
  if v_count < 1 or v_count > 12 then raise exception 'Workout prescription must contain 1 to 12 exercises' using errcode = 'P0001'; end if;
  select min(ce.training_direction), max(ce.training_direction) into v_direction, v_item_direction from public.plan_workout_exercises we join public.cfg_exercises ce on ce.id = we.exercise_id where we.workout_id = p_workout_id;
  if v_direction is null then
    v_direction := case when v_workout.name like '%推%' or lower(v_workout.name) like '%push%' then 'push' when v_workout.name like '%拉%' or lower(v_workout.name) like '%pull%' then 'pull' when v_workout.name like '%蹲%' or lower(v_workout.name) like '%squat%' then 'squat' else null end;
  elsif v_item_direction is distinct from v_direction then raise exception 'Workout exercise direction is incompatible' using errcode = 'P0001'; end if;
  if v_direction is null then raise exception 'Workout exercise direction is incompatible' using errcode = 'P0001'; end if;
  select coalesce(jsonb_agg(jsonb_build_object('exercise_id', we.exercise_id, 'order_index', we.order_index, 'target_sets', we.target_sets, 'target_reps', we.target_reps, 'target_weight', we.target_weight) order by we.order_index), '[]'::jsonb) into v_before from public.plan_workout_exercises we where we.workout_id = p_workout_id;
  ...
  update public.plan_workouts set prescription_revision = prescription_revision + 1, updated_at = now() where id = p_workout_id returning prescription_revision into v_new_revision;
  select coalesce(jsonb_agg(jsonb_build_object('exercise_id', we.exercise_id, 'order_index', we.order_index, 'target_sets', we.target_sets, 'target_reps', we.target_reps, 'target_weight', we.target_weight) order by we.order_index), '[]'::jsonb) into v_after from public.plan_workout_exercises we where we.workout_id = p_workout_id;
  insert into public.ops_workout_revision_events (user_id, program_id, workout_id, previous_revision, new_revision, before_prescription, after_prescription) values (v_user_id, v_workout.program_id, p_workout_id, v_new_revision - 1, v_new_revision, v_before, v_after);
  return jsonb_build_object('workout_id', p_workout_id, 'prescription_revision', v_new_revision, 'exercises', v_after);
end;
$$;
